...
enum class PageTableState { FULL, NORMAL, EMPTY };
...
public:
    ...
    PageTable(backend::Device &device, uint32_t table_page_num, vk::DeviceSize page_size)
        : SparseResources(table_page_num, page_size), device_(device)
    {
    ...
    void allocate_pages()
    {
        vkGetBufferMemoryRequirements(
            static_cast<VkDevice>(device_.get_handle()),
            buffers_[0]->get_handle(),
            &memory_requirements_);
