...
MeshDrawPreparationPass::MeshDrawPreparationPass(GpuScene &gpu_scene, sg::Camera &camera) :
...
void MeshDrawPreparationPass::execute(backend::CommandBuffer &command_buffer, RenderFrame &active_frame, std::vector<ShaderBindable> input_bindables)
{
	...
	command_buffer.bind_buffer(gpu_scene_.get_mesh_draws_buffer(), 0, gpu_scene_.get_mesh_draws_buffer().get_size(), 0, 2, 0);
	...
	command_buffer.bind_buffer(gpu_scene_.get_draw_command_buffer(), 0, gpu_scene_.get_draw_command_buffer().get_size(), 0, 4, 0);
	command_buffer.bind_buffer(gpu_scene_.get_draw_counts_buffer(), 0, gpu_scene_.get_draw_counts_buffer().get_size(), 0, 5, 0);
	command_buffer.bind_buffer(gpu_scene_.get_mesh_bounds_buffer(), 0, gpu_scene_.get_mesh_bounds_buffer().get_size(), 0, 6, 0);
