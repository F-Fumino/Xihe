...
void RenderGraph::execute(bool present)
{
	...
	auto begin_time = begin_timer.stop();
	LOGI("Begin time: {} ms", begin_time);
	...
	auto frame_time = frame_timer.stop();
	LOGI("Frame time: {} ms", frame_time * 1000.0f);
}
